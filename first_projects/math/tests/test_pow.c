...
START_TEST(static_base_dyn_exp_from_0_to_1) {
  long double base = 4.2525865;
  for (long double exp = 0; exp < 1; exp += 0.01) {
    long double result = s21_pow(base, exp);
    long double expected = pow(base, exp);
    ck_assert_ldouble_eq_tol(result, expected, ACCURACY);
  }
...
START_TEST(static_base_dyn_exp_from_1_to_7000) {
  long double base = 4.2525865;
  for (long double exp = 1; exp < 7000; exp += 10) {
    long double result = s21_pow(base, exp);
    long double expected = pow(base, exp);
    ck_assert_ldouble_eq_tol(result, expected, ACCURACY);
  }
...
START_TEST(dyn_base_from_0_to_1_static_exp_upper_1) {
  long double exp = 4.2525865;
  for (long double base = 0; base < 1; base += 0.01) {
    long double result = s21_pow(base, exp);
    long double expected = pow(base, exp);
    ck_assert_ldouble_eq_tol(result, expected, ACCURACY);
  }
...
START_TEST(dyn_base_from_0_to_1_static_exp_lower_1) {
  long double exp = 0.045;
  for (long double base = 0; base < 1; base += 0.01) {
    long double result = s21_pow(base, exp);
    long double expected = pow(base, exp);
    ck_assert_ldouble_eq_tol(result, expected, ACCURACY);
...
START_TEST(dyn_base_from_1_to_7000_static_exp_upper_1) {
  long double exp = 4.2525865;
  for (long double base = 1; base < 7000; base += 10) {
    long double result = s21_pow(base, exp);
    long double expected = pow(base, exp);
    ck_assert_ldouble_eq_tol(result, expected, ACCURACY);
  }
...
START_TEST(dyn_base_from_1_to_7000_static_exp_lower_1) {
  long double exp = 0.045;
  for (long double base = 1; base < 7000; base += 10) {
    long double result = s21_pow(base, exp);
    long double expected = pow(base, exp);
...
Suite *pow_suite(void) {
  ...
  tcase_add_test(tc_pow, static_base_dyn_exp_from_0_to_1);
  tcase_add_test(tc_pow, static_base_dyn_exp_from_1_to_7000);
  tcase_add_test(tc_pow, dyn_base_from_0_to_1_static_exp_upper_1);
  tcase_add_test(tc_pow, dyn_base_from_0_to_1_static_exp_lower_1);
  tcase_add_test(tc_pow, dyn_base_from_1_to_7000_static_exp_upper_1);
  tcase_add_test(tc_pow, dyn_base_from_1_to_7000_static_exp_lower_1);

  suite_add_tcase(
      pow_suite,
      tc_pow);  // добавление тест-кейса "pow" в набор тестов "pow_suite"
